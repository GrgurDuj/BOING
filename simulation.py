...
cloudcoverage_matrix = np.loadtxt("cloudcoverage_map_yearly_average.csv", delimiter=',', dtype=float)
...
#AOI matrix
# counter = 0
# points_proj = np.empty([160000,2])
# #print(AOI_matrix.shape)
# for i in range(int(AOI_matrix.shape[0])):
#     for j in range(int(AOI_matrix.shape[1])):
#         if(AOI_matrix[i,j] != 0):
#             point = [(0.5*m.pi -(m.pi/AOI_matrix.shape[0] * i)), (2*m.pi/AOI_matrix.shape[1] * j)+m.pi]
#             point_proj = STXY(point[0],point[1])
#             points_proj[counter,0] = point_proj[0]
#             points_proj[counter,1] = point_proj[1]
#             counter += 1
#plt.scatter(points_proj[:,0],points_proj[:,1],color='red', s = 0.5, marker='.', linewidths=0)

#Cloud coverage matrix
counter = 0
points_proj_red = np.empty([7000000,2])
points_proj_orange = np.empty([7000000,2])
points_proj_yellow = np.empty([7000000,2])
points_proj_green = np.empty([7000000,2])
points_proj_blue = np.empty([7000000,2])
for i in range(cloudcoverage_matrix.shape[0]):
    for j in range(cloudcoverage_matrix.shape[1]):
        if(cloudcoverage_matrix[i,j] < 0.2):
            point = [(0.5*m.pi -(m.pi/cloudcoverage_matrix.shape[0] * i)), (2*m.pi/cloudcoverage_matrix.shape[1] * j)]
            point_proj = STXY(point[0],point[1])
            points_proj_red[counter,0] = point_proj[0]
            points_proj_red[counter,1] = point_proj[1]
            counter += 1
        elif(cloudcoverage_matrix[i,j] < 0.4):
            point = [(0.5*m.pi -(m.pi/cloudcoverage_matrix.shape[0] * i)), (2*m.pi/cloudcoverage_matrix.shape[1] * j)]
            point_proj = STXY(point[0],point[1])
            points_proj_orange[counter,0] = point_proj[0]
            points_proj_orange[counter,1] = point_proj[1]
            counter += 1
        elif(cloudcoverage_matrix[i,j] < 0.6):
            point = [(0.5*m.pi -(m.pi/cloudcoverage_matrix.shape[0] * i)), (2*m.pi/cloudcoverage_matrix.shape[1] * j)]
            point_proj = STXY(point[0],point[1])
            points_proj_yellow[counter,0] = point_proj[0]
            points_proj_yellow[counter,1] = point_proj[1]
            counter += 1
        elif(cloudcoverage_matrix[i,j] < 0.8):
            point = [(0.5*m.pi -(m.pi/cloudcoverage_matrix.shape[0] * i)), (2*m.pi/cloudcoverage_matrix.shape[1] * j)]
            point_proj = STXY(point[0],point[1])
            points_proj_green[counter,0] = point_proj[0]
            points_proj_green[counter,1] = point_proj[1]
            counter += 1
        else:
            point = [(0.5*m.pi -(m.pi/cloudcoverage_matrix.shape[0] * i)), (2*m.pi/cloudcoverage_matrix.shape[1] * j)]
            point_proj = STXY(point[0],point[1])
            points_proj_blue[counter,0] = point_proj[0]
            points_proj_blue[counter,1] = point_proj[1]
            counter += 1
print(counter)
plt.scatter(points_proj_red[:,0], points_proj_red[:,1],color='red', s = 0.1, marker='.', linewidths=0)
plt.scatter(points_proj_orange[:,0], points_proj_orange[:,1],color='orange', s = 0.1, marker='.', linewidths=0)
plt.scatter(points_proj_yellow[:,0], points_proj_yellow[:,1],color='yellow', s = 0.1, marker='.', linewidths=0)
plt.scatter(points_proj_green[:,0], points_proj_green[:,1],color='green', s = 0.1, marker='.', linewidths=0)
plt.scatter(points_proj_blue[:,0], points_proj_blue[:,1],color='blue', s = 0.1, marker='.', linewidths=0)

fig.savefig("cloudcoverage_matrix.jpg", dpi=2000)
plt.show()
#DayNightMatrix = DayNightMatrix(time)
