...
def satelliteEllipse(inclination, right_ascension, time):
    ...
    longitude = clockangle + right_ascension - (
                (time / day) * 2 * m.pi)  # rotation of the Earth beneath a prograde orbit
    if (
            latitude > 0.5 * m.pi):  # if the inclination overflows 0.5*pi, the angle is reversed and rotated to the other side of the globe
        latitude = m.pi - latitude
        longitude += m.pi
    if (latitude < -0.5 * m.pi):
        latitude = -(m.pi + latitude)
...
for i in range(counter):
    ...
    plt.scatter(point[0], point[1], color='blue', s=0.02)

fig.savefig("AOI_2_TEST_deg", dpi=2000)
#plt.show()
print('finished2222')